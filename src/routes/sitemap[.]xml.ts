  ...
  server: {
    handlers: {
      GET: async () => {
        ...
        // Published blog posts. Read with the public key so the sitemap can only
        // ever list what an anonymous visitor could open.
        const blogRouteId = "/blog/$slug";
        if (isSitemapRouteIncluded(router.routesById[blogRouteId])) {
          ...
          for (let offset = 0; ; ) {
            ...
            for (const row of data) {
              ...
              if (path) entries.push({ path, lastmod: String(row.updated_at).slice(0, 10) });
            }
